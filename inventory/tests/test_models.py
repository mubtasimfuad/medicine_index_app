...
from inventory.models import GenericName, MedicineCategory, MedicineForm, Manufacturer
...
def test_generic_name_crud(admin_user):
    ...
    ), f"Failed to create: {response.data}"
    generic_id = response.data["data"]["id"]

    # Retrieve
    response = client.get(f"/api/generic-names/{generic_id}/")
    print(
        "RETRIEVE GenericName Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to retrieve: {response.data}"
    assert response.data["data"]["name"] == "Test Generic"

    # Update
    response = client.put(
        f"/api/generic-names/{generic_id}/", data={"name": "Updated Generic"}
    )
    print("UPDATE GenericName Response:", response.status_code, response.data)  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to update: {response.data}"
    ...
    ), f"Failed to delete: {response.data}"
...
def test_medicine_category_crud(admin_user):
    ...
    print(
        "CREATE MedicineCategory Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_201_CREATED
    ), f"Failed to create: {response.data}"
    ...
    print(
        "RETRIEVE MedicineCategory Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to retrieve: {response.data}"
    ...
    # Update
    response = client.put(
        f"/api/categories/{category_id}/", data={"name": "Updated Category"}
    )
    print(
        "UPDATE MedicineCategory Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to update: {response.data}"
    ...
    print(
        "DELETE MedicineCategory Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_204_NO_CONTENT
    ), f"Failed to delete: {response.data}"
...
def test_medicine_form_crud(admin_user):
    ...
    print("CREATE MedicineForm Response:", response.status_code, response.data)  
    assert (
        response.status_code == status.HTTP_201_CREATED
    ), f"Failed to create: {response.data}"
    ...
    print(
        "RETRIEVE MedicineForm Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to retrieve: {response.data}"
    ...
    # Update
    response = client.put(
        f"/api/forms/{form_id}/", data={"description": "Updated Syrup form"}
    )
...
def test_manufacturer_crud(admin_user):
    ...
    print(
        "RETRIEVE Manufacturer Response:", response.status_code, response.data
    )  
    assert (
        response.status_code == status.HTTP_200_OK
    ), f"Failed to retrieve: {response.data}"
    ...
    # Update
    response = client.put(
        f"/api/manufacturers/{manufacturer_id}/", data={"contact_info": "098-765-4321"}
    )
